...
Some names aren’t yours to change: a public API, a serialized field, a DB column, a wire key, an inherited signature, a spec term. Callers and stored data depend on the exact string, so the wrong one is a breaking change. 
